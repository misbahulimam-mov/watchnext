...
let selectedType="All",selectedGenre="All",liveResults=[];
const esc=s=>String(s||"").replace(/[&<>"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));
function card(x){const bg=x.image?' style="background-image:linear-gradient(180deg,#0000,#000b),url(\\''+x.image+'\\');background-size:cover;background-position:center"':'';return '<article class="card" data-name="'+esc(x.name)+'"><div class="poster"'+bg+'><span class="type">'+x.type+'</span><span class="initial">'+esc(x.name[0])+'</span></div><div class="info"><h3>'+esc(x.name)+' <span class="rating">★ '+esc(x.rating||"—")+'</span></h3><div class="meta">'+esc(x.year||"")+' · '+esc(x.genre||"")+'</div><p class="why">'+esc(x.why||"Explore this title and discover something new.")+'</p></div></article>'}
function render(){let source=liveResults.length?liveResults:items;let q=document.querySelector("#search").value.toLowerCase();let filtered=source.filter(x=>(selectedType==="All"||x.type===selectedType)&&(selectedGenre==="All"||selectedGenre==="All"||x.genre===selectedGenre)&&(!q||x.name.toLowerCase().includes(q)||x.genre.toLowerCase().includes(q)));document.querySelector("#recommendations").innerHTML=filtered.slice(0,8).map(card).join("")||'<p style="color:#999">No matches yet. Try another title.</p>';document.querySelector("#trendingGrid").innerHTML=(liveResults.length?liveResults.slice(0,4):items.slice(3,7)).map(card).join("");document.querySelector("#gemsGrid").innerHTML=items.slice(1,5).map(card).join("");bindCards()}
function bindCards(){document.querySelectorAll(".card").forEach(c=>c.onclick=()=>{let x=[...liveResults,...items].find(x=>x.name===c.dataset.name);openModal(x)})}
function openModal(x){document.querySelector("#modalBody").innerHTML='<div class="eyebrow">'+esc(x.type)+' · '+esc(x.year||"")+'</div><h2>'+esc(x.name)+'</h2><div class="tags">'+(x.tags||[]).map(t=>'<span class="tag">'+esc(t)+"</span>").join("")+'</div><p>'+esc(x.desc||"Live metadata loaded from the title database.")+'</p><strong>★ '+esc(x.rating||"—")+'</strong><p><b>Why you’ll like it:</b> '+esc(x.why||"Explore the title and see where it leads.")+'</p>';document.querySelector("#modal").classList.add("open")}
async function liveSearch(q){
if(!q){liveResults=[];render();return}
const tasks=[];
if(selectedType==="All"||selectedType==="Movies")tasks.push(
fetch("https://imdb.iamidiotareyoutoo.com/search?q="+encodeURIComponent(q)+"&v=1")
...
.then(data=>{
 ...
 return rows.slice(0,8).map(v=>{
  const id=v.imdb_id||v.imdbId||v.id||v.tt||v.tconst||v.title_id||"";
  const title=v.title||v.name||v.primaryTitle||v.originalTitle||v.l||"Untitled";
  const year=v.year||v.release_year||v.releaseYear||v.y||v.startYear||"";
  const rating=v.rating||v.imdb_rating||v.imdbRating||v.averageRating||v.r||"";
  const poster=v.poster||v.image||v.posterUrl||v.poster_url||v.photo||"";
  const posterUrl=poster||((id&&String(id).startsWith("tt"))?"https://imdb.iamidiotareyoutoo.com/photo/"+id:"");
  return {name,title,type:"Movies",genre:v.genre||v.genres||"Movie",rating,year,image:posterUrl,desc:v.plot||v.overview||v.description||v.desc||"",tags:Array.isArray(v.genres)?v.genres:(v.genre?[v.genre] :[]),why:"A live movie result matched to your search.",imdbId:id};
 })
}).catch(()=>[])
);
...
liveResults=(await Promise.all(tasks)).flat();
if(!liveResults.length && selectedType==="Movies") liveResults=items.filter(x=>x.type==="Movies");
render();
}
...
document.querySelector("#close").onclick=()=>document.querySelector("#modal").classList.remove("open");document.querySelector("#modal").onclick=e=>{if(e.target.id=="modal")e.currentTarget.classList.remove("open")};document.querySelector("#themeBtn").onclick=()=>document.body.classList.toggle("light");render();