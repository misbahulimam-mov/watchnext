...
function openModal(x){
 document.querySelector("#modalBody").innerHTML='<div class="eyebrow">'+esc(x.type)+' · '+esc(x.year||"")+'</div><h2>'+esc(x.name)+'</h2><div class="tags">'+(x.tags||[]).map(t=>'<span class="tag">'+esc(t)+'</span>').join("")+'</div><p>'+esc(x.desc||"Live metadata loaded from the title database.")+'</p><strong>★ '+esc(x.rating||"—")+'</strong><p><b>Why you’ll like it:</b> '+esc(x.why||"Explore the title and see where it leads.")+'</p>';
 document.querySelector("#modal").classList.add("open");
}
